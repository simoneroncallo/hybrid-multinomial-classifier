...
def run_simulation(arch, mode, dataset):
    ...
    # --- DATASET --- #
    (X, Y), (XAll, YAll) = load_dataset(dataset, download = download,\
                                        labels = labelmask)

    if mode == "one_vs_rest" or mode == "one_vs_one":
        # Split dataset
        (num_classes, num_models), train, val, test = split_versus_dataset(X, Y, XAll, YAll, mode, balanced_dataset,\
            trainval_ratio, rng, show_population = False, device = device, dtype = dtype)

    elif mode == "tree":
        # Generate tree
        (num_classes, _), (X0, Y0), test = normalize_dataset(X, Y, XAll, YAll, device = device, dtype = dtype)
        tree, partition, depth = get_tree(labelmask, num_classes, rng) 
        (Xtest, Ytest) = test
        
        # Split dataset
        (num_models, idx2labels, tree_map), train, val = split_tree_dataset(X0, Y0, Xtest, Ytest, tree, depth, rng, verbose = False)
    ...
    for idx in tqdm(range(num_models)):
        ...
        (_, _), history_train[idx,:,:], history_val[idx,:,:] = models[idx].fit(train_loader[idx],val_loader[idx],\
                                                                                   num_epochs,loss_function,optimizers[idx]) # Train

    # --- PLOT --- #
    plot_history(history_train, history_val, show_legend = False)
    plt.savefig(f"./data/{arch}_{mode}_{dataset}_history.png")
    

    # --- INFERENCE --- #
    objects = models, tree, partition, tree_map
    acc = get_accuracy(objects, Xtest, Ytest, num_classes, num_models, mode, device = device, dtype = dtype) # Per-class accuracies
...
def main():
    ...
    # Explicit settings
    # arch = "classical" # Available {classical, quantum}
    # mode = "tree" # Available {one_vs_rest, one_vs_one, tree}
